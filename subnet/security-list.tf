...
dynamic "egress_security_rules" {
    ...
    content {
      ...
      dynamic "tcp_options" {
        for_each = rule.value.min == null ? tomap({}) : rule.value.protocol == "tcp" || rule.value.protocol == null ?  tomap({rule.value.min = rule.value.max}) : tomap({})
        iterator = port
        content {
        min = port.key
...
# Ingress Rules
 ...
 dynamic "ingress_security_rules" {
    ...
    content {
      ...
      dynamic "tcp_options" {
        for_each = rule.value.protocol == "tcp" || rule.value.protocol == null ? rule.value.min != null ? rule : object() : object()
        iterator = rule
        content {
        min = rule.value.min 
        max = rule.value.max != null ? rule.value.max : rule.value.min
